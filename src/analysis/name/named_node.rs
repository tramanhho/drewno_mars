...
// impl NamedNodeVector for Vec<Rc<Stmt>> {
//     fn named_unparse_vec(self: Vec<Rc<Stmt>>, unparser: &mut NamedUnparser, join: &'static str) -> String {
//         return self.iter()
...
// impl NamedNodeVector for Vec<Rc<Exp>> {
//     fn named_unparse_vec(self: Vec<Rc<Exp>>, unparser: &mut NamedUnparser, join: &'static str) -> String {
//         return self.iter()
//             .map(|arg| arg.named_unparse(unparser))
//             .collect::<Vec<String>>().join(join);
//     }
...
//     fn named_unparse(&self, unparser: &mut NamedUnparser) -> String {
//         unparser.add_entry(self.id.to_string(), SymbolKind::Variable {var_type: self.var_type.clone()}, &self.id.span.borrow());
//         match &self.init_val {
...
// impl Decl {
//     fn process_class_field(&self, class_id: Rc<Id>, unparser: &mut NamedUnparser, field_outputs: &mut Vec<String>)  {
//         use Decl::*;
//         match self {
//             VarDecl(ref x) => {
//                 let x = x.clone();
//                 let kind = SymbolKind::Variable { var_type: x.var_type.clone() };
//                 unparser.add_class_entry(class_id.to_string(), x.id.to_string(), kind, &class_id.span.borrow());
...
// fn process_fn(unparser: &mut NamedUnparser, args: Vec<Rc<FormalDecl>>, ret: Rc<Type>) -> (&mut NamedUnparser, SymbolKind) {
//     let mut arg_map : HashMap<String, Rc<Type>> = HashMap::new();
//     unparser.scope += 1;
...
//         // unparser.add_entry(id.to_string(), argkind, &id.span); // <- we don't need? bc we process it in print_args later
//     }
...
//         unparser.add_entry(self.id.to_string(), value.clone(), &self.id.span.borrow());
...
//                 unparser.add_entry(id.to_string(), SymbolKind::Variable {var_type: formal_type.clone()}, &id.span.borrow());
...
// impl NamedNode for LineStmtKind {
//     fn named_unparse(&self, unparser: &mut NamedUnparser) -> String {
//         use LineStmtKind::*;
//         match self {
...
// fn get_loc_named_string(name: Rc<Loc>, unparser: &mut NamedUnparser) -> (String, &mut NamedUnparser) {
//     let name = name.kind.get_id();
//     get_id_named_string(name, unparser)
// }

// fn get_id_named_string(name: Rc<Id>, unparser: &mut NamedUnparser) -> (String, &mut NamedUnparser) {
...
//         // match *self.kind.clone() {
//         //     LocKind::Id(x) => {
//         //         let (output, _unparser) = get_id_named_string(x, unparser);
//         //         output
//         //     },
...
//         //                 let (output, _unparser) = get_loc_named_string(base_class, unparser);
//         //                 output
//         //             },
//         //             "Loc" => {
//         //                 if current == "" {
